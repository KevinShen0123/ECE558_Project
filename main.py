...
if len(sys.argv) == 1:#The code for server side
   ...
   connectionhandler=threading.Thread(target=handle_connection(server_socket=server_socket))
   connectionhandler.start()
   def client_handler(client_socket):
       try:
          server_connection.handle_client(client_socket)
       except:
   ...
   client_entry_thread=threading.Thread(target=client_p2p_connection.client_entry,args=((server_ip,server_port,sys.argv[3],int(sys.argv[4]))))
